...
目标很简单：让 AI 知道游戏里正在发生什么，并提供独立的游戏聊天体验，不把游戏聊天和日常聊天混在一起。当前仓库里的浏览器页面主要用于演示和诊断，不代表普通玩家以后必须开网页。

**目前版本只读取游戏状态，不控制 Sim。**

> 当前状态：只读链路已经在 **The Sims 4 1.128.90.1030** 完成第一次真实游戏 Smoke Test。
> 这证明脚本能加载、读取真实游戏状态并送到本地程序，但还不是普通玩家下载后直接玩的正式版本。
...
| 在真实《The Sims 4》中加载验证 | 1.128.90.1030 只读 Smoke Test 已通过 |
| AI 控制 Sim | 没有开放 |
| 普通玩家一键安装 | 还没有 |
...
这个页面目前是开发演示和诊断入口，不是未来普通玩家必须常开的聊天页面。停止时按 `Ctrl+C`。
